...
from kye.models import User, Task, Chat
...
def officer(request):
    ...
    return render(request, 'dashboard/officer/home.html', context)
...
def create_user(request):
    ...
    if request.method == 'POST':
            ...
            print(userType)
            if userType == "Officer":
                a = User(first_name=first_name, last_name=last_name, username=username, email=email, password=password, is_officer=True)
                a.save()
                ...
                a = User(first_name=first_name, last_name=last_name, username=username, email=email, password=password, is_manager=True)
                a.save()
                messages.success(request, 'Member was created successfully!')
                return redirect('aluser')    
            else:
...
def create_user_form(request):
    ...
    return render(request, 'dashboard/admin/add_user.html', choice)
...
def case_form(request):
    return render(request, 'dashboard/officer/add_case.html')    
...
class CaseListView(LoginRequiredMixin, ListView):
    model = Task
    template_name = 'dashboard/officer/task_list.html'
...
class PenCase(LoginRequiredMixin, ListView):
    model = Task
    template_name = 'dashboard/officer/pending.html'
    context_object_name = 'tasks'
    paginate_by = 10
...
class DeadlineTask(LoginRequiredMixin, ListView):
    model = Task
    template_name = 'dashboard/admin/deadline.html'
    context_object_name = 'tasks'
    paginate_by = 10


    def get_queryset(self):
        return Task.objects.all().filter(status="deadline")


class UCreateChat(LoginRequiredMixin, CreateView):
    form_class = ChatForm
    model = Chat
    template_name = 'dashboard/officer/chat_form.html'
...
class ACreateChat(LoginRequiredMixin, CreateView):
    form_class = ChatForm
    model = Chat
    template_name = 'dashboard/admin/chat_form.html'
...
class UListChat(LoginRequiredMixin, ListView):
    model = Chat
    template_name = 'dashboard/officer/chat_list.html'
    paginate_by = 10
...
class AListChat(LoginRequiredMixin, ListView):
    model = Chat
    template_name = 'dashboard/admin/chat_list.html'
    paginate_by = 10
...
class DeadCaseTask(LoginRequiredMixin, ListView):
    model = Task
    template_name = 'dashboard/officer/deadline.html'
    context_object_name = 'tasks'
    paginate_by = 10
...
class CompCase(LoginRequiredMixin, ListView):
    model = Task
    template_name = 'dashboard/officer/completed.html'
    context_object_name = 'tasks'
    paginate_by = 10
...
def search(request):
    ...
    if query is None:
        return redirect('dashboard')
                ...
                qs9 =models.Task.objects.all().filter(id__icontains=query)


                files = itertools.chain(qs1,qs3, qs4, qs5, qs6, qs7, qs8, qs9)
...
def osearch(request):
    ...
    if query is None:
        return redirect('officer')
                ...
                if files:
                    return render(request,'dashboard/officer/result.html',{'files':files,'word':word})
                return render(request,'dashboard/officer/result.html',{'files':files,'word':word})
